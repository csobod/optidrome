...
# encoding: utf-8
##############################################################################
#     prescriptionSelector.py - Select a prescription by patient
#
##############################################################################
...
class PrescriptionSelectForm(npyscreen.FormBaseNew):
    "Prescription selector and FCRUD options."
    ...
    def create(self):
        ...
        self.how_exited_handers[npyscreen.wgwidget.EXITED_ESCAPE]  = self.exitPrescriptionSelector   # Escape exit
        
        # Form title - Screen title line
        pname, version = config.pname, config.program_version
        self.form_title = pname + " " + version + " - Prescription selector"
        self.today = self.get_today()
        self.formTitle=self.add(bs.MyTextfield, name="PrescriptionTitle", value=None, relx=0, rely=0, \
    ...
    def exitPrescriptionSelector(self):
        "Escape key was pressed: isinstance(self, PrescriptionSelectForm) = True; we always come from the OptionField."
        get_out = False
        if self.statusLine.value != self.optionsLiteral:    # it's not the options statusline; it's the numeral field
            ...
            self.hide_detail()
            self.ask_option()
        else:           # we come from the OptionField, we must exit to the patient form
            self.inputOpt.value = ""
            self.inputOpt.update(clear=True)
            self.ask_option()  # in case we'll return from the patient form
            get_out = True
